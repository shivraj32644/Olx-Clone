import {Box, Button, FormControl, FormLabel, Grid, HStack, Icon, Img, Input, InputGroup, InputLeftElement, InputRightElement, Select, Tab, TabList, TabPanel, TabPanels, Tabs, textDecoration, VStack }from "@chakra-ui/react"
...
const {name,value}=e.target
  ...
  //  name=="set_price"? numValue=Number(value):
    ...
    name=="set_price"? setInputPrice(value):
    name=="year_buy" ? setInputYear(value):
    name=="km_driven" ? setInputKm(value):
    name=="key_feature" ? setInputKeyFeature(value):
    name=="description" ? setInputDescription(value):

    name=="key_feature"?settextAreaCountKeyFeature(value.length):
    name=="description" ?settextAreaCountDescription(value.length):

    textAreaCountKeyFeature>0 && textAreaCountKeyFeature<10 && name=="key_feature" ? setErrorKeyFeature(true):setErrorKeyFeature(false)
    textAreaCountDescription>0 && textAreaCountDescription<10 && name=="description" ? setErrorDescription(true):setErrorDescription(false)
    name=="km_driven"&& inputKm=="" ? setErrorKm(true):setErrorKm(false)
    name=="year_buy"&& inputYear==""? setErrorYear(true):setErrorYear(false)
    name=="set_price"&& inputPrice==""?setErrorPrice(true):setErrorPrice(false)

    name=="year_buy"||name=="km_driven"|| name=="set_price" ? setSellingFormObj({...sellingFormObj,[name]:value}):setSellingFormObj({...sellingFormObj,[name]:value})
...
// need local storage for user_id after login --------------install uuidv4()-------------------<
    ...
    const handleBrandObj=(brandInput)=>{
      ...
      setSellingFormObj({...sellingFormObj,name:brandObj.name, car_model: brandObj.car_model, car_varient:brandObj.car_varient})

    }

    const handleLocationObj=(locationInput)=>{
      setLocationObj(locationInput)
      setSellingFormObj({...sellingFormObj, ads_location:{city:"Bhopal",  state:locationObj.state,  neighbour:locationObj.neighbour}})
          ...
          <VStack fontFamily="sans-serif" fontSize="16px" border="1px solid #cecece"  borderRadius="5px" display="block">
            ...
            <FormControl >
              <Box w="430px" py="30px" px="32px" >
                ...
                <YearInput isError={errorYear} input={inputYear} handleInputChange={handleInputChange}/>

                <Box mb="15px">
                 <Box fontSize="12px" mb="1px" >Fuel *</Box>
                 <Box fontSize="12px" display="flex"  flexWrap="wrap" onClick={handleFormData}>
                   <Button {...fuelBoxStyle} name="fuel">CNG & Hybrid</Button>
                   <Button {...fuelBoxStyle} name="fuel">Diesel</Button>
                   <Button {...fuelBoxStyle} name="fuel">LPG</Button>
                   <Button {...fuelBoxStyle} name="fuel">Petrol</Button>
                   <Button {...fuelBoxStyle} name="fuel">Electric</Button>
                 </Box>
                </Box>
                ...
                <Box mb="15px">
                 ...
                 <Box fontSize="12px" display="flex"  flexWrap="wrap"  onClick={handleFormData}>
                  <Button {...fuelBoxStyle} name="transmission">Automatic</Button>
                  <Button {...fuelBoxStyle} name="transmission">Manual</Button>
                 </Box>
                </Box>

                <KmInput handleInputChange={handleInputChange} input={inputKm} isError={errorKm}/>

                <Box mb="15px">
                  <Box fontSize="12px" mb="1px" >No. of owners *</Box>
                  <Box fontSize="12px" display="flex"  flexWrap="wrap" onClick={handleFormData}>
                    <Button {...fuelBoxStyle} name="no_of_oweners" value={1}>1st</Button>
                    <Button {...fuelBoxStyle} name="no_of_oweners" value={2}>2nd</Button>
                    <Button {...fuelBoxStyle} name="no_of_oweners" value={3}>3rd</Button>
                    <Button {...fuelBoxStyle} name="no_of_oweners" value={4}>4th</Button>
                    <Button {...fuelBoxStyle} name="no_of_oweners" value={5}>4+</Button>
                  </Box>
                </Box>

                <Box>
                  <AddTitle  handleInputChange={handleInputChange} input={inputKeyFeature} textAreaCount={textAreaCountKeyFeature} isError={errorKeyFeature}/>
                  <DescriptionInput handleInputChange={handleInputChange} input={inputDescription} textAreaCount={textAreaCountDescription} isError={errorDescription}/>
                </Box>
                <hr />
                <PriceInput input={inputPrice} isError={errorPrice} handleInputChange={handleInputChange} />
                <hr my="15px" />
                
                <FormControl mb="20px">
                 <Box fontSize="16px" fontWeight="bold" lineHeight="24px" mt="15px" pb="10px" >UPLOAD UPTO 6 PHOTOS</Box>
                 <Grid templateColumns='repeat(4, 1fr)' gap="8px">  
                          <Box  h="100px" w="100px" border="2px solid black" display="flex" justifyContent="center" alignItems="center" cursor="pointer" >
                           ...
                           <Input type="file" id="productImg" name="img" 
                            style={{display:"none", visibility:"none"}}
                            multiple
                            accept="image/png,img/jpg,img/jpeg,img/webp"
                             />  
                         </Box>  
                         <Box  h="100px" w="100px" border="2px solid #cecece" display="flex" justifyContent="center" alignItems="center" cursor="pointer" >
                         <label for="productImg">
                          <VStack h="90px" w="90px"pt="20%" >
                           <Icon as={TbCameraPlus}  h="28px" w="28px"/> 
                           <Text fontSize='12px'>Add Photos</Text>
               ...
               />  
                ...
                <FormControl my="15px">
                  ...
                  <HStack mb="15px" w="100%">
                    <Box h="100px" w="100px" overflow="hidden" p="3px" borderRadius="50%" border="1px solid #cecece" mr="20px">
                      <Img src={user.img_url} />
                    </Box>
                    <Box >
                     <FormLabel fontSize="12px" mb="1px" >Name *</FormLabel>
                     <Box  w="230px" p="8px" fontSize="14px" borderRadius="3px" border="1px solid #cecece">{user.full_name}</Box>
                    </Box>
                  </HStack>
                  <Box display="flex" justifyContent="space-between">
                   <Box fontSize="12px" display="flex">Email: <Box ml="45px">{user.email_id}</Box> </Box>
                  </Box>
                </FormControl>
                <hr/>
